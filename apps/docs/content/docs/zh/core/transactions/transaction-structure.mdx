...
![展示消息头部三个组成部分的示意图](/assets/docs/core/transactions/message_header.png)

### 账户地址

[`account_keys`](https://github.com/anza-xyz/solana-sdk/blob/clock%40v2.2.3/message/src/legacy.rs#L168)
字段是一个紧凑编码的公钥数组。每个条目标识交易指令中至少使用一次的账户。该数组必须包含所有账户，并遵循以下严格排列顺序：
...
<Callout>
  这种严格的排序方式使得 `account_keys` 数组能够与消息 [`header`](#header)
  中的三个计数值相结合，从而无需为每个账户单独存储权限元数据标志，即可确定每个账户的权限。头部计数值将数组划分为上述四个权限组。
</Callout>

![显示账户地址数组顺序的示意图](/assets/docs/core/transactions/compat_array_of_account_addresses.png)

### 最近区块哈希

`recent_blockhash` 字段是一个 32 字节的哈希值，用于两个目的：

1. **时间戳**：证明交易是最近创建的。
2. **去重**：防止同一笔交易被处理两次。

区块哈希在 150 个 slot 后过期。如果交易到达时区块哈希已不再有效，交易将被拒绝并返回
_rs`BlockhashNotFound`_，除非它是有效的[持久化 nonce 交易](/docs/core/transactions/durable-nonces)。

<Callout>
  [`getLatestBlockhash`](/docs/rpc/http/getlatestblockhash) RPC
  方法允许您获取当前区块哈希以及该区块哈希有效的最后区块高度。
</Callout>
...
2. [`accounts`](mention:account-indexes)：`account_keys`
   数组中的索引数组，用于指定传递给程序的账户。
3. [`data`](mention:instruction-data)：字节数组，包含 instruction
   data 的判别符和序列化参数。
...
交易使用紧凑编码方案进行序列化。所有可变长度数组（签名、账户密钥、指令）均以紧凑 u16 长度编码作为前缀。该格式对 0-127 的值使用 1 个字节，对更大的值使用 2-3 个字节。
...
| `account_indices`  | `num_accounts` x 1 字节 | 账户密钥索引            |
| `data_len`         | 1-3 字节 (compact-u16)  | instruction data 长度   |
| `data`             | `data_len` 字节         | 不透明 instruction data |

### 大小计算

给定 _rs`PACKET_DATA_SIZE`_ = 1,232 字节，可用空间计算如下：
...
下图展示了交易与指令如何协同工作，使用户能够与网络进行交互。在本示例中，SOL 从一个账户转移到另一个账户。

发送方账户的
[元数据](/docs/core/instructions/instruction-structure#account-metadata)
表明它必须为该交易签名。这允许 System
Program 扣除 lamport。发送方和接收方账户都必须是可写的，以便其 lamport 余额发生变化。为执行此指令，发送方的钱包将发送包含其签名的交易以及含有 SOL 转账指令的消息。

![SOL 转账示意图](/assets/docs/core/transactions/sol-transfer.svg)

交易发送后，System Program 会处理转账指令并更新两个账户的 lamport 余额。
...
System
Program 转账可向**任意**账户添加 lamport。协议层面不会检查接收方是否能将 SOL 转出。lamport 只能由账户的所属程序转出，因此向
[token mint](/docs/tokens/basics/create-mint)、某个程序或您不控制的
[PDA](/docs/core/pda) 发送 SOL **有永久丢失资金的风险**
—— 只有所属程序指定的授权方才能将其取回。发送到
[token account](/docs/tokens/basics/create-token-account)
的 SOL 只能由该账户的所有者取回，发送方无法取回。

SPL **token** 转账具有一定的自我保护机制：Token
Program 会拒绝账户与预期 mint 不匹配的转账。原生 **SOL**
转账没有此类保护，因此发送方在签名前必须验证接收方。完整的分类逻辑请参阅[验证地址](/docs/payments/send-payments/verify-address)。

</Callout>

以下示例展示了与上述示意图相关的代码。请参阅 System Program 的
...
由于 SOL 转账可以成功发送至任何账户，请在签名前核实收款方身份。获取该账户信息，仅向 System
Program 钱包（或未注资的[链上地址](/docs/payments/send-payments/verify-address#on-curve-addresses)）发送；拒绝向铸币账户、token
...
<Callout>
  此代码片段用于检查原生 SOL 的收款方。如需完整的分类逻辑（同时处理 SPL
  代币发送，包括 token account、ATA 及 Token-2022），请参阅 [Verify
  Address](/docs/payments/send-payments/verify-address)。
</Callout>

## 获取交易详情

提交后，可使用交易签名及 [getTransaction](/docs/rpc/http/gettransaction)
RPC 方法检索交易详情。

<Callout>
  您也可以通过 [Solana Explorer](https://explorer.solana.com) 查找该交易。
</Callout>
...
原始响应通过索引标识账户，并将内部（CPI）指令存储为编码的二进制数据。要将这些解析为地址并遍历完整的指令树，请参阅[交易内省](/docs/core/transactions/transaction-introspection)。
