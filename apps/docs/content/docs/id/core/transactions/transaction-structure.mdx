...
description:
  Anatomi transaksi Solana termasuk tanda tangan, format pesan, header, alamat
  akun, blockhash, instruksi terkompilasi, encoding biner, dan anggaran ukuran.
...
<Callout type="info" title="Ringkasan">
  Transaksi memiliki tanda tangan + pesan. Pesan berisi header, alamat akun,
  blockhash terbaru, dan instruksi terkompilasi. Ukuran serialisasi maksimal:
  1.232 byte.
</Callout>
...
- [`signatures`](mention:signatures): Array tanda tangan
...
tersebut mencakup array [`signatures`](#signatures) dan struct
[`message`](#message).
...
tangan diperlukan untuk setiap [akun penandatangan](#account-addresses) yang
direferensikan oleh instruksi transaksi.

Setiap tanda tangan dihasilkan oleh private key. Tempat penyimpanan key tersebut
— keypair lokal, cloud HSM atau KMS, atau layanan wallet terkelola — merupakan
keputusan desain untuk lingkungan produksi. Lihat
[Penandatanganan di Produksi](/docs/core/transactions/signing-in-production).

Tanda tangan pertama dalam array milik **fee payer**, yaitu akun yang membayar
[biaya dasar dan biaya prioritisasi transaksi](/docs/core/fees/fee-structure#base-fee).
Tanda tangan pertama ini juga berfungsi sebagai **ID transaksi**, yang digunakan
untuk mencari transaksi di jaringan. ID transaksi umumnya disebut sebagai
**tanda tangan transaksi**.

Persyaratan fee payer:

- Harus menjadi akun pertama dalam pesan (indeks 0) dan merupakan penanda
  tangan.
...
- Harus memiliki cukup lamport untuk menanggung
  `rent_exempt_minimum + total_fee`; jika tidak, transaksi akan gagal dengan
...
- [`recent_blockhash`](mention:recent-blockhash): Sebuah
  [blockhash](#recent-blockhash) yang berfungsi sebagai cap waktu transaksi
...
- [`num_readonly_signed_accounts`](mention:num_readonly_signed_accounts): Jumlah
  akun bertanda tangan yang bersifat hanya-baca.
- [`num_readonly_unsigned_accounts`](mention:num_readonly_unsigned_accounts):
  Jumlah akun tidak bertanda tangan yang bersifat hanya-baca.
...
![Diagram yang menunjukkan tiga bagian dari header pesan](/assets/docs/core/transactions/message_header.png)
...
mengidentifikasi sebuah akun yang digunakan oleh setidaknya satu instruksi
transaksi. Array tersebut harus menyertakan setiap akun dan harus mengikuti
urutan ketat berikut:

1. Penanda tangan + Dapat ditulis
2. Penanda tangan + Hanya baca
3. Bukan penanda tangan + Dapat ditulis
4. Bukan penanda tangan + Hanya baca

<Callout>
  Urutan ketat ini memungkinkan array `account_keys` digabungkan dengan tiga
  hitungan dalam [`header`](#header) pesan untuk menentukan izin bagi setiap
...
![Diagram yang menunjukkan urutan array alamat akun](/assets/docs/core/transactions/compat_array_of_account_addresses.png)

### Blockhash terbaru

Kolom `recent_blockhash` adalah hash 32-byte yang memiliki dua tujuan:

1. **Stempel waktu**: membuktikan bahwa transaksi dibuat baru-baru ini.
...
saat transaksi tiba, transaksi akan ditolak dengan _rs`BlockhashNotFound`_,
kecuali jika merupakan
[transaksi durable nonce](/docs/core/transactions/durable-nonces) yang valid.
...
adalah array berenkode ringkas dari struct
[`CompiledInstruction`](https://github.com/anza-xyz/solana-sdk/blob/clock%40v2.2.3/message/src/compiled_instruction.rs#L21).
Setiap _rs`CompiledInstruction`_ mereferensikan akun berdasarkan indeks ke dalam
array `account_keys`, bukan berdasarkan public key lengkap. Ini berisi:

<WithMentions>

1. [`program_id_index`](mention:program-id-index): Indeks ke dalam
   `account_keys` yang mengidentifikasi program yang akan dipanggil.
2. [`accounts`](mention:account-indexes): Array indeks ke dalam `account_keys`
   yang menentukan akun yang akan diteruskan ke program.
3. [`data`](mention:instruction-data): Array byte yang berisi discriminator
   instruksi dan argumen terserialisasi.
...
![Array ringkas dari Instruksi](/assets/docs/core/transactions/compact_array_of_ixs.png)

## Format biner transaksi

Transaksi diserialisasi menggunakan skema enkoding ringkas. Semua array dengan
panjang variabel (tanda tangan, kunci akun, instruksi) diberi awalan enkoding
panjang compact-u16. Format ini menggunakan 1 byte untuk nilai 0-127 dan 2-3
byte untuk nilai yang lebih besar.
...
| `account_keys`            | `num_account_keys` x 32 byte | Kunci publik                                           |
| `recent_blockhash`        | 32 byte                      | Blockhash                                              |
| `num_instructions`        | 1-3 byte (compact-u16)       | Jumlah instruksi                                       |
| `instructions`            | variabel                     | Array dari [instruksi yang dikompilasi](#instructions) |
...
Diberikan _rs`PACKET_DATA_SIZE`_ = 1.232 byte, ruang yang tersedia dapat
dihitung:
...
Diagram di bawah ini menunjukkan bagaimana transaksi dan instruksi bekerja sama
untuk memungkinkan pengguna berinteraksi dengan jaringan. Dalam contoh ini, SOL
ditransfer dari satu akun ke akun lainnya.

[Metadata](/docs/core/instructions/instruction-structure#account-metadata) akun
pengirim menunjukkan bahwa akun tersebut harus menandatangani transaksi. Hal ini
memungkinkan System Program untuk mengurangi lamport. Baik akun pengirim maupun
penerima harus dapat ditulis, agar saldo lamport mereka dapat berubah. Untuk
menjalankan instruksi ini, dompet pengirim mengirimkan transaksi yang berisi
tanda tangannya dan pesan yang berisi instruksi transfer SOL.

![Diagram transfer SOL](/assets/docs/core/transactions/sol-transfer.svg)

Setelah transaksi dikirim, System Program memproses instruksi transfer dan
memperbarui saldo lamport dari kedua akun.

![Diagram proses transfer SOL](/assets/docs/core/transactions/sol-transfer-process.svg)

<Callout type="warn" title="Verifikasi penerima sebelum mengirim SOL">

Transfer System Program menambahkan lamport ke akun **mana pun**. Tidak ada
pemeriksaan di tingkat protokol bahwa penerima dapat memindahkan SOL kembali.
Lamport hanya dapat dipindahkan keluar oleh program pemilik akun, sehingga
mengirim SOL ke [token mint](/docs/tokens/basics/create-mint), sebuah program,
atau [PDA](/docs/core/pda) yang tidak Anda kendalikan **berisiko kehilangan dana
secara permanen** — hanya otoritas yang ditentukan oleh program pemilik yang
dapat mengembalikannya. SOL yang dikirim ke
[token account](/docs/tokens/basics/create-token-account) hanya dapat dipulihkan
oleh pemilik akun tersebut, tidak pernah oleh pengirim.

Transfer **token** SPL sebagian bersifat melindungi diri sendiri: Token Program
menolak transfer yang akunnya tidak cocok dengan mint yang diharapkan. Transfer
**SOL** native tidak memiliki perlindungan seperti itu, sehingga pengirim harus
memverifikasi penerima sebelum menandatangani. Lihat
[Verifikasi Alamat](/docs/payments/send-payments/verify-address) untuk logika
klasifikasi lengkapnya.

</Callout>

Contoh di bawah ini menunjukkan kode yang relevan dengan diagram di atas. Lihat
[fungsi `transfer`](https://github.com/anza-xyz/agave/blob/v3.1.8/programs/system/src/system_processor.rs#L210)
...
<WithMentions>
Kode di bawah ini menampilkan output dari cuplikan kode sebelumnya.
Formatnya berbeda antar SDK,
namun perhatikan bahwa setiap instruksi mengandung informasi yang sama dan diperlukan.
...
Karena transfer SOL berhasil ke akun mana pun, periksa penerima sebelum
menandatangani. Ambil data akun dan kirim hanya ke dompet System Program (atau
alamat
[on-curve](/docs/payments/send-payments/verify-address#on-curve-addresses) yang
belum didanai); tolak mint, token account, program, dan PDA yang tidak Anda
kendalikan.
...
<Callout>
  Cuplikan ini memeriksa penerima SOL native. Untuk klasifikasi lengkap yang
  juga menangani pengiriman token SPL (token account, ATA, Token-2022), lihat
  [Verify Address](/docs/payments/send-payments/verify-address).
</Callout>

## Mengambil detail transaksi

Setelah pengiriman, ambil detail transaksi menggunakan tanda tangan transaksi
dan metode RPC [getTransaction](/docs/rpc/http/gettransaction).
...
Respons mentah mengidentifikasi akun berdasarkan indeks dan menyimpan instruksi
inner (CPI) sebagai blob yang dienkode. Untuk mengubahnya menjadi alamat dan
menelusuri seluruh pohon instruksi, lihat
[Introspeksi Transaksi](/docs/core/transactions/transaction-introspection).
