...
Ogni firma è prodotta da una chiave privata. La posizione di tale chiave — un
keypair locale, un HSM o KMS cloud, oppure un servizio di wallet gestito — è una
decisione di progettazione per la produzione. Vedere
...
[commissione base e la commissione di priorità](/docs/core/fees/fee-structure#base-fee)
della transazione. Questa prima firma funge anche da **ID transazione**,
utilizzato per individuare la transazione sulla rete. L'ID transazione è
comunemente denominato **firma della transazione**.
...
- [`num_readonly_signed_accounts`](mention:num_readonly_signed_accounts): Numero
  di account firmati in sola lettura.
- [`num_readonly_unsigned_accounts`](mention:num_readonly_unsigned_accounts):
  Numero di account non firmati in sola lettura.
...
è un array di chiavi pubbliche codificato in formato compatto. Ogni voce
...
<Callout>
  ...
  tre contatori nell'[`header`](#header) del messaggio per determinare i
  permessi di ciascun account senza memorizzare flag di metadati per singolo
  account. I contatori dell'header suddividono l'array nei quattro gruppi di
  permessi elencati sopra.
</Callout>
...
si tratti di una valida
[transazione con nonce durevole](/docs/core/transactions/durable-nonces).

<Callout>
  Il metodo RPC [`getLatestBlockhash`](/docs/rpc/http/getlatestblockhash)
  consente di ottenere il blockhash corrente e l'ultima altezza di blocco alla
...
è un array a codifica compatta di struct
...
1. [`program_id_index`](mention:program-id-index): Indice nell'array
   `account_keys` che identifica il programma da invocare.
2. [`accounts`](mention:account-indexes): Array di indici nell'array
   `account_keys` che specifica gli account da passare al programma.
...
Le transazioni vengono serializzate utilizzando uno schema di codifica compatta.
Tutti gli array a lunghezza variabile (firme, chiavi degli account, istruzioni)
sono preceduti da una codifica della lunghezza in formato compact-u16. Questo
formato utilizza 1 byte per valori da 0 a 127 e 2-3 byte per valori più grandi.
...
Ogni istruzione compilata viene serializzata come segue:
...
| `data`             | `data_len` byte         | instruction data opachi       |
...
Il diagramma seguente mostra come le transazioni e le istruzioni collaborano per
...
al System Program di detrarre lamport. Sia l'account mittente che quello
...
transazione contenente la propria firma e il messaggio con l'istruzione di
trasferimento SOL.

![Diagramma del trasferimento SOL](/assets/docs/core/transactions/sol-transfer.svg)
...
![Diagramma del processo di trasferimento SOL](/assets/docs/core/transactions/sol-transfer-process.svg)

<Callout type="warn" title="Verifica il destinatario prima di inviare SOL">

Un trasferimento tramite System Program aggiunge lamport a **qualsiasi**
account. Non esiste alcun controllo a livello di protocollo che garantisca che
il destinatario possa ritrasferire il SOL. I lamport possono essere spostati
solo dal programma proprietario dell'account, quindi inviare SOL a un
[token mint](/docs/tokens/basics/create-mint), a un programma o a un
[PDA](/docs/core/pda) che non controlli **comporta il rischio di perdita
permanente dei fondi** — solo un'autorità specificata dal programma proprietario
può restituirli. Il SOL inviato a un
[token account](/docs/tokens/basics/create-token-account) è recuperabile solo
dal proprietario di quell'account, mai dal mittente.

I trasferimenti di **token** SPL sono parzialmente autoprotetti: il Token
Program rifiuta un trasferimento i cui account non corrispondono al mint
previsto. I trasferimenti nativi di **SOL** non dispongono di tale protezione,
quindi il mittente deve verificare il destinatario prima di firmare. Consulta
[Verifica Indirizzo](/docs/payments/send-payments/verify-address) per la logica
di classificazione completa.

</Callout>

L'esempio seguente mostra il codice rilevante per i diagrammi precedenti.
Consulta la funzione
...
<WithMentions>
Il codice seguente mostra l'output dei precedenti frammenti di codice.
Il formato varia a seconda degli SDK,
ma si noti che ogni istruzione contiene le stesse informazioni richieste.
...
### Verifica il destinatario prima del trasferimento

Poiché un trasferimento SOL va a buon fine su qualsiasi account, verifica il
destinatario prima di firmare. Recupera l'account e invia solo a un wallet
System Program (o a un indirizzo
[on-curve](/docs/payments/send-payments/verify-address#on-curve-addresses) non
finanziato); rifiuta mint, token account, programmi e PDA che non controlli.
...
<Callout>
  Questo snippet verifica i destinatari SOL nativi. Per la classificazione
  completa che gestisce anche gli invii di token SPL (token account, ATA,
...
La risposta grezza identifica gli account tramite indice e memorizza le
istruzioni interne (CPI) come blob codificati. Per risolverli in indirizzi e
percorrere l'intero albero delle istruzioni, consulta
[Introspezione delle Transazioni](/docs/core/transactions/transaction-introspection).
